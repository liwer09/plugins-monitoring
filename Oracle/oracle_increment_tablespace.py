...
def check_oracle_connection():
    ...
    try:
        ...
        cursor.execute(f'''
        select * from espacio_usuario
        where DESC_USUARIO = '{ORACLE_TABLE}'
        ''')
        result = cursor.fetchone()
        #print(result[0])
        print(f"Usuario {result[0]}, Espacio {result[1]} Bytes, Incremento {result[2]} Bytes | espacio={result[1]} incremento={result[2]}")
